...
public abstract class ArrayList<T> implements ListADT<T>, Iterable<T>
{
    ...
    {
        list = Arrays.copyOf(list, list.length * 2);//use the Arrays util package to expand the list
    }
    ...
    public T removeLast() throws EmptyCollectionException
    {
        ...
        {
            T result; //create result
            rear--; //decrement rear so it points to the correct element
            result = list[rear];//store rear
            list[rear] = null;//remove link to last element
            modCount++;
    ...
    public T removeFirst() throws EmptyCollectionException
    {
        ...
        } else
        {
            ...
            for (int scan=0; scan < rear; scan++)//move all elements up one element
            {
    ...
    public T[] toArray()
    {
        return Arrays.copyOfRange(list, 0, rear);
    }
    ...
    public T first() throws EmptyCollectionException
    {
        ...
        {
            return list[front];//simply return the front or first element
        }
    }
    ...
    public T last() throws EmptyCollectionException
    {
        ...
        {
            return list[rear-1];//simply return the last element
        }
    }
    ...
    {
        return (front == rear && rear == 0); //added so that there is an additional constraint
    }
    ...
    public int size()
    {
        return rear; //size of the list is just the value of rear, rear points to the element behind the last element so this works
    }
    ...
    public String toString()
    {
        ...
        for (int scan=0; scan < rear; scan++) //custom to string which displays the array in a brackets fashion
            if(scan < rear-1)
    ...
    private class ArrayListIterator implements Iterator<T>
    {
        ...
        /**
         * Returns true if this iterator has at least one element before the current element
         * @return true if there is a previous element
         * @throws ConcurrentModificationException
         */
        public boolean hasPrevious() throws ConcurrentModificationException //implemented because I thought I needed but did not need
        {
            if (iteratorModCount != modCount)
                throw new ConcurrentModificationException();
        
            return (front < current); //if current is ahead of front it means that there is an element previous to current
        }
    
        /**
         * Moves the pointer one space backwards in the list and returns the previous element
         * @return the previous element if possible
         * @throws ConcurrentModificationException
         */
        public T previous() throws ConcurrentModificationException //not used at the moment
        {
            if(!hasPrevious())
                throw new NoSuchElementException();
        
            current--; //decrement current so it points to the previous element
        
            return list[current]; //return current which is now the previous element
        }
    }
